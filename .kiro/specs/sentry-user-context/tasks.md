...
- [ ] 1. SentryUserProvider の作成とルートレイアウトへの統合
...
- [ ] 1.2 SentryUserProvider のテストを作成する
...
- [ ] 1.3 ルートレイアウトに SentryUserProvider を配置する
  - `app/layout.tsx` に SentryUserProvider を追加し、既存のProviderツリーに統合する
  - _Requirements: 1.1, 1.2_

- [ ] 2. サーバーサイドのSentryユーザーコンテキスト設定
- [ ] 2.1 (P) ダッシュボードレイアウトでSentryユーザーコンテキストを設定する
  - `app/dashboard/layout.tsx` の認証済みユーザー取得後にSentryユーザーコンテキストを設定する
  - 送信するユーザー情報はSupabaseユーザーID（UUID）のみとする
  - _Requirements: 1.3, 3.2, 3.3_

- [ ] 2.2 (P) ログアウトServer ActionでSentryユーザーコンテキストをクリアする
  - `app/auth/actions.ts` の signOut 関数内でSentryユーザーコンテキストをクリアする
  - セッション破棄の前にクリアを実行する
  - _Requirements: 2.1, 3.3_

- [ ] 2.3 サーバーサイドのSentryユーザーコンテキスト設定のテストを作成する
