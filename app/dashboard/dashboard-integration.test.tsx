...
// useSearchParams が毎回同じ参照を返すようにキャッシュ
// 新しいインスタンスを返すと useEffect が毎レンダリングで再実行されるため
let cachedParams: URLSearchParams | null = null;
let cachedGuild: string | null = null;

function getMockSearchParams() {
  if (!cachedParams || cachedGuild !== mockNavState.guild) {
    cachedParams = new URLSearchParams();
    cachedParams.set("view", "month");
    cachedParams.set("date", "2025-12-05");
    if (mockNavState.guild) {
      cachedParams.set("guild", mockNavState.guild);
    }
    cachedGuild = mockNavState.guild;
  }
  return cachedParams;
}
