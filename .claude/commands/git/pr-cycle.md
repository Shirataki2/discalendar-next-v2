...
3. **状態に基づく判定**:
   - mainブランチにいて、未コミットの変更がある → **PR作成フロー**
   - mainブランチにいて、変更がない → **ステータス報告**（何もすることがない）
   - featureブランチにいて、PRが存在しない + 変更がある → **PR作成フロー**
   - featureブランチにいて、PRが存在する → PRのレビューコメントを確認:
     - レビューコメントがある → **レビュー修正フロー**
     - レビューコメントがない + 未コミット変更がある → **PR作成フロー**（追加コミット）
     - レビューコメントがない + 変更がない → **ステータス報告**
...
- mainブランチにいる場合: `git checkout -b <branch-name>`
- featureブランチにいる場合: 既存ブランチをそのまま使用
