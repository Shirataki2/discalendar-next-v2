...
- **ルート (`/`)** - Next.js Webアプリ（フロントエンド）
- **`packages/bot`** - Discord Bot（`@discalendar/bot`）
...
# パッケージ
packages/
  bot/            # Discord Bot (@discalendar/bot)
    src/
      commands/   # スラッシュコマンド（create, help, init, invite, list）
      events/     # Discordイベントハンドラ
      services/   # ビジネスロジック（guild-service, event-service）
      tasks/      # 定期タスク（notify, presence）
      types/      # 型定義
      utils/      # ユーティリティ（logger, embeds, datetime）
```
